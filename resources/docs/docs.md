...
The `home-routes` handler in the `rchain-scan.routes.home` namespace
...
The `home-page` function will in turn call the `rchain-scan.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/rchain_scan/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `rchain-scan.handler` namespace:
...
Request middleware functions are located under the `rchain-scan.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `rchain-scan.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `rchain-scan.db.core` in some other namespace.
